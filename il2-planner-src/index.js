...
import icons from './icons'; // (L); <- fix this
...
var map, mapTiles, mapConfig, drawnItems, drawnMarkers, hiddenLayers, frontline, drawControl, selectedMapIndex, V;

var state = {
  units: win.localStorage?.getItem('units') || 'metric',
  style: win.localStorage?.getItem('style') || 'cb',
...
function changeUnits(units) {
  state.units = units;
  win.localStorage?.setItem('units', units);
...
function changeStyle(style) {
  state.style = style;
  win.localStorage?.setItem('style', style);
  win.location?.reload();
}
...
function selectMap(selectedMapConfig) {
  ...
  if (newIndex !== selectedMapIndex) {
    ...
    win.location.hash = selectedMapConfig.hash;
...
function importMapState(saveData, keepExisting) {
  var keep = keepExisting && saveData.mapHash === win.location?.hash;
  if (!keep) {
    clearMap();
  }
...
function publishMapState() {
  if (state.streaming) {
    ...
    webdis.publish(
      ...
      win.escape(JSON.stringify(saveData))
    );
  }
}
...
export default function initializeMap() {
  initializeControls();
  V = new Validatinator(content.validatinatorConfig);
  if (win.location?.hash !== '' && !util.isAvailableMapHash(win.location?.hash, content.maps)) {
    ...
    if (conf.apiUrl === 'NONE') {
      ...
      if (win.location?.hash.substring(0, 10) === '#json-url=' && win.location?.hash.length > 10) {
        url = /*cors_api_url +*/ win.location?.hash.slice(10);
      } else {
        switch (win.location?.hash) {
          case '#combatbox':
      ...
      url = conf.apiUrl + '/servers/' + win.location?.hash.substr(1);
    }
  ...
  mapConfig = util.getSelectedMapConfig(win.location?.hash, content.maps);
  selectedMapIndex = mapConfig.selectIndex;
  /* // Debugging
...
*/
            ...
            onShow: function (e) {
              ...
              L.DomEvent.on(importInput, 'change', function (evt) {
                ...
                reader.onload = function (evt) {
                  ...
                  if (evt.target.error) {
                    win.alert('Error while reading file');
                    return;
                  }
            ...
            map.openModal({
              ...
              onShow: function (e) {
                ...
                L.DomEvent.on(e.modal._container.querySelector('.modal-print'), 'click', function () {
                  var summaryPrint = win.open('', '', 'height=700, width=700');
            ...
            map.openModal({
              ...
              onShow: function (e) {
                ...
                L.DomEvent.on(document.getElementById('stream-start-confirm-button'), 'click', function () {
                  ...
                  var mapState = win.escape(JSON.stringify(exportMapState()));
  ...
  win.addEventListener('il2:streamerror', function (e) {
    if (!state.connected) {
      return;
    }
  ...
  win.addEventListener('il2:streamupdate', function (e) {
    if (!state.connected) {
      return;
    }
